...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    while unvisitedCorners:
        ...
        closestCornerDistance = util.manhattanDistance(previousPos, closestCorner)

        for corner in unvisitedCorners[1:]:
            distance = util.manhattanDistance(previousPos, corner)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    all_nodes = [position] + unvisitedFoods
    graph = []
    for i in range(len(all_nodes)):
        for j in range(i + 1, len(all_nodes)):
            node1 = all_nodes[i]
            node2 = all_nodes[j]
            weight = util.manhattanDistance(node1, node2)
            graph.append((node1, node2, weight))

    if len(all_nodes) <= 2:
        return graph[0][2] if graph else 0
